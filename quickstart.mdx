...
async def main():
    # Pass a provider name as a string: "ollama", "gemini", "groq", "azure", "anthropic"
...
The `on_token` callback fires for every token in the streaming response. `response['content']` still holds the complete assembled text when the call returns.
